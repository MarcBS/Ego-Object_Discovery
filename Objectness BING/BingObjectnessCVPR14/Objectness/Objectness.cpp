...
Objectness::Objectness(DataSet *voc, double base, int W, int NSS, int numPerSz)
	: _voc(voc)
	, _base(base)
	, _W(W)
	, _NSS(NSS)
...
Objectness::Objectness(void):
	...
	_base(2),  _logBase(log(_base)), // base for window size quantization
	_W(8), // As described in the paper: #Size, Size(_W, _H) of feature window. 
	_NSS(2), // Size for non-maximal suppress
	_numPerSz(130), // Number of window candidates created per image size
	_maxT(cvCeil(log(500.)/_logBase)), _minT(cvCeil(log(10.)/_logBase)), _numT(_maxT - _minT + 1) // The minimal and maximal dimensions of the template

{}

void Objectness::initTest(CStr &testPath){
	//DataSet * new_voc;
	//new_voc = static_cast<DataSet *>(_voc);


	_voc->initTest(testPath);
	_voc->loadAnnotationsTest();

	//_voc = new_voc;
}

void Objectness::readyTraining(){
	//DataSet * new_voc;
	//new_voc = static_cast<DataSet *>(_voc);

	_voc->annoPathW = _voc->annoPathTrain;
	_voc->imgPathW = _voc->imgPathTrain;
}

void Objectness::readyTesting(){
	//DataSet * new_voc;
	//new_voc = static_cast<DataSet *>(_voc);

	_voc->annoPathW = _voc->annoPathTest;
	_voc->imgPathW = _voc->imgPathTest;
}
...
void Objectness::getObjBndBoxes(CMat &img3u, ValStructVec<float, Vec4i> &valBoxes, int numDetPerSize)
{
	...
	/*// Resize image if any side bigger than 500 pixels!
	int maxSize = 500;
	double props = 1.0;
	if(img3u.rows > img3u.cols){
		if(img3u.rows > maxSize){
			props = img3u.rows / maxSize;
		}
	}else{
		if(img3u.cols > maxSize){
			props = img3u.cols / maxSize;
		}
	}
	Size size((int)(img3u.rows/props), (int)(img3u.cols/props));
	Mat newimg3u;//dst image
	resize(img3u,newimg3u,size);//resize image
	*/

	// Tmp resize SenseCam images
	Size size((int)((double)img3u.cols/_props), (int)((double)img3u.rows/_props));
	Mat newimg3u;//dst image
	resize(img3u,newimg3u,size);//resize image

	CV_Assert_(filtersLoaded() , ("SVM filters should be initialized before getting object proposals\n"));
	vecI sz;
	predictBBoxSI(newimg3u, valBoxes, sz, numDetPerSize, false);
...
void Objectness::trainObjectness()
{
...
// Get potential bounding boxes for all test images
void Objectness::getObjBndBoxesForTestsFast(vector<vector<Vec4i>> &_boxesTests)
{
...
#pragma omp parallel for
	for (int i = 0; i < TestNum; i++)
#ifdef PRELOAD_IMGS
		getObjBndBoxes(imgs3u[i], boxesTests[i], _numPerSz);
#else
		getObjBndBoxes(imread(format(_S(_voc->imgPathW), _S(_voc->testSet[i]))), boxesTests[i], _numPerSz);
#endif
...
// Get potential bounding boxes for a single test image
void Objectness::getObjBndBoxesForNoValidationTestFast(CStr &imagepath, vector<vector<Vec4i>> &_boxesTests)
{

	loadTrainedModel();
	//illustrate();

	vecS listImages = CmFile::loadStrList(imagepath + "names.txt");

	const int TestNum = listImages.size();
	vector<ValStructVec<float, Vec4i>> boxesTests;
	boxesTests.resize(TestNum);
...
#pragma omp parallel for
	for (int i = 0; i < TestNum; i++)
		imgs3u[i] = imread(imagepath + listImages[i] + ".jpg");
#endif // PRELOAD_IMGS
...
#pragma omp parallel for
	for (int i = 0; i < TestNum; i++)
		...
		getObjBndBoxes(imgs3u[i], boxesTests[i], _numPerSz);
#else
		getObjBndBoxes(imread(imagepath + listImages[i] + ".jpg"), boxesTests[i], _numPerSz);
#endif
...
#pragma omp parallel for
	for (int i = 0; i < TestNum; i++){
		CStr fName = _bbResDir + listImages[i];
		ValStructVec<float, Vec4i> &boxes = boxesTests[i];
		FILE *f = fopen(_S(fName + ".txt"), "w");
		fprintf(f, "%d\n", boxes.size());
		for (size_t k = 0; k < boxes.size(); k++)
